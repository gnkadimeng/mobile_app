...
const GmsLoginScreen = ({ onNavigateBack, onLoginSuccess }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://10.114.21.31:5000/GMsLogin", {
        email,
        password
      });
      ...
      if (response.data.message === "GMsLogin successful") {
        ...
        console.log("GM Login successful for:", userData.email);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.formContainer}>
        <Text style={styles.title}>GRANT MANAGEMENT SIGN IN</Text>
